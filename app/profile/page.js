...
export default function ProfilePage() {
  ...
  // Fetch user orders after user is loaded
  useEffect(() => {
    if (!user) return;
        ...
        {ordersLoading ? (
          ...
          <div className="space-y-4">
            {orders.map((order) => {
              ...
              return (
                <div key={order._id || order.id} className="border border-gray-200 rounded-xl p-6 hover:shadow-md transition-shadow">
                  <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <span className="text-xl font-bold" style={{ color: '#2e2e2e' }}>
                        {((order.totalValue || order.amount || 0))}DA
                      </span>
                    </div>
                  </div>
                  {/* Order Items */}
