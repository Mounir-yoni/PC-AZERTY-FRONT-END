...
const api = axios.create({
  ...
  baseURL: process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.azertycomputer.com/api/v1',
});
...
export async function addOrder({ address, name, phone, wilaya, place, wilayaObj, preserveCart = false }) {  
  try {
    const cart = getCart();
    ...
    console.log(wilayaObj)
    let shipping = 0;
    if(place === 'office'){
      shipping = wilayaObj.pricetooffice;
    }
    else{
      shipping = wilayaObj.pricetohome;
    }
    const totalValue = shipping + products.reduce((total, item) => {
    ...
    // Only clear cart if not preserving it (for direct orders)
    if (!preserveCart) {
...
export function addsliderimage(data){
  try {
    return api.post('/homepageslider', data, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      },
    })
...
export function getsliderimages() {
  try {
    return api.get('/homepageslider')
    .then(res => res.data.data)
    .catch(err => {
      console.error("Error getting slider images:", err);
      throw err.response?.data || err;
    });
  } catch (error) {
...
export function addsliderimages(data) {
  try {
    ...
    console.error("Error in addsliderimages:", error);
    throw error;
  }
} 
...
export function deletesliderimage(id) {
  try {
    return api.delete(`/homepageslider/${id}`, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      },
    })
...
export async function getcategorysbyadmin(){
    try {
    const res = await api.get(`/admin/all-category`,
      {headers: {
        Authorization: `Bearer ${getToken()}`,
      }}
    );
    ...
    console.error("Error getting categories by admin:", err);
    throw err.response?.data || err;
  }
}
...
export async function getwilayas(){
  try {
    const res = await api.get('/wilayas');
    return res.data;
  } catch (err) {
    throw err.response?.data || err;
  }
...
export async function addwilaya(Data) {
  try {
    const res = await api.post('/wilayas', Data, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      }
    });
...
export async function editwilaya(id, Data) {
  try {
    const res = await api.put(`/wilayas/${id}`, Data, {
      headers: {
        Authorization: `Bearer ${getToken()}`,
      }
    });
    ...
    console.error("Error editing wilaya:", error);
    throw error.response?.data || error;
  }
}